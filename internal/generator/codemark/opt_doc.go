...
func (o optDocResourcer) Options() []*optv1.Option {
	return makeOpts(_optDocResource,
		...
		mustMakeOpt("summary", OptDocSummary(""), _unique, optv1.TargetNamed),
	)
}
...
func (o optDocResourcer) newDoc(named *infov1.NamedInfo) *docv1.Option {
	doc := &docv1.Option{}
	optsRes := named.Options().Filter(_domain, _optDocResource)
	for _, opts := range optsRes {
		for _, opt := range opts {
			switch t := opt.(type) {
			case OptDocDesc:
				doc.Desc = strings.TrimSpace(string(t))
			case OptDocSummary:
				doc.Summary = strings.TrimSpace(string(t))
			}
		}
	}
	return doc
...
func (o optDocResourcer) createFile(pkg *packages.Package, docs map[types.Object]*docv1.Option) (*genv1.Artifact, error) {
	var b bytes.Buffer
	fmt.Fprintf(&b, "// CODE GENERATED BY CODEMARK\n")
